...
def switch_json(message):
    print (message)
    text_all = [json.loads(sentence)['text'] for sentence in message] # 순서 매우 중요~!
    
    print (pg.get_translate_list(text_all, 'ko'))
    # ko_message = message[:]
    # en_message = message[:]
    # jp_message = message[:]
...
def processincomsumer(message):
    ...
    if img is not None:
        result = reader.readtext(img, decoder = 'beamsearch', beamWidth=5, width_ths=2, paragraph= False, batch_size = 100000, output_format='json_specific_and_relative_pos')
        print (switch_json(result))
    else:
        print ('Error img is null')
